...
class Traffic():
    def __init__(self, nodes_num, type, capacity):
        ...
        # read traffic diretory 
        if self.type.startswith('DIR'):
            self.dir = sorted(listdir("./traffic-matrices"))
    ...
    def dir_traffic(self):
        while len(self.dir) > 0:
            tm_xml = self.dir.pop(0)
            tm = np.zeros((23,23))

            tree = ET.parse("./traffic-matrices/" + tm_xml)
            root = tree.getroot()

            for src in root[1].findall('src'):
                for dst in src.findall('dst'):
                    tm[int(src.get("id"))-1][int(dst.get("id"))-1] = dst.text
            
            np.fill_diagonal(tm, -1)
            return tm
        return False
